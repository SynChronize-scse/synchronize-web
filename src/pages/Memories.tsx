...
export default function FlowingPathScroll() {
    ...
    const handleResize = () => {
      if (containerRef.current && contentRef.current && svgRef.current) {
        ...
        containerRef.current.style.height = `${
          horizontalScrollDistance + verticalScrollDistance + containerHeight
        }px`;
  ...
  return (
    <div>
      ...
      <div ref={containerRef} className="relative bg-[#141414] hidden md:block">
        ...
        >
          <div className="absolute top-0 left-0 w-full h-full">
            {pathPoints.map((point, index) => {
              ...
              const visibilityProgress = Math.max(
                ...
                Math.min(
                  1,
                  (scrollProgress * pathLength - appearThreshold) /
                    (pathLength * 0.05)
                )
              );
          ...
          >
            <g clipPath="url(#clip0_1_2)">
              <rect
                width={7963 * SCALE_FACTOR}
                height={385 * SCALE_FACTOR}
                fill="#141414"
              />
              <path
                ref={pathRef}
