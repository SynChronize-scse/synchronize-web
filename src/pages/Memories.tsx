          ...
          {pathPoints.map((point, index) => {
            ...
            return (
              <div
                ...
                style={{
                  left: `${point.x}px`,
                  top: `${point.y + 30 * SCALE_FACTOR}px`,
                  opacity: visibilityProgress,
                  transform: `translate(-50%, -50%) scale(${1.5 + visibilityProgress * 1.5})`,
                  zIndex: 20,
                }}
              >
                <div className="flex items-start">
                  <div className="w-28 h-28 overflow-hidden rounded-lg bg-primary">
                    <img
                  ...
                  <div className="ml-2 bg-black bg-opacity-75 p-0.5 rounded-md max-w-[160px]">
                    <p className="text-white text-[7px] leading-tight">
                      {imageDescriptions[imageIndex].split(' ').reduce((acc, word, i) => {
                        if (i > 0 && i % 3 === 0) acc += '\n'
                        acc += word + ' '
                        return acc
                      }, '').trim()}
